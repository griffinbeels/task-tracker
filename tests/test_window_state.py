...
MINIMIZED = {"width": 237, "height": 39, "x": -32000, "y": -32000, "on_top": True}
...
def test_load_falls_back_to_defaults_when_the_file_is_corrupt():
    ...
    assert restored["on_top"] is True


def test_load_restores_a_position_on_the_monitor_left_of_the_primary():
    write_saved_state({"width": 760, "height": 1018, "x": -812, "y": 396, "on_top": True})

    assert window_state.load(SCREENS) == {
        "width": 760, "height": 1018, "x": -812, "y": 396, "on_top": True,
    }
...
def test_load_restores_a_window_hanging_off_the_edge_of_its_screen():
    ...
    write_saved_state({"width": 420, "height": 900, "x": 2500, "y": 200, "on_top": True})
...
def test_load_keeps_on_top_even_when_the_geometry_is_discarded():
    write_saved_state({**MINIMIZED, "on_top": False})

    assert window_state.load(SCREENS)["on_top"] is False
...
def test_save_records_a_position_that_is_on_a_screen():
    window_state.save({"width": 760, "height": 1018, "x": -812, "y": 396, "on_top": True}, SCREENS)
...
def test_save_still_records_on_top_when_the_geometry_is_rejected():
    window_state.save({"width": 760, "height": 1018, "x": -812, "y": 396, "on_top": True}, SCREENS)

    window_state.save({**MINIMIZED, "on_top": False}, SCREENS)

    assert read_saved_state()["on_top"] is False
    assert read_saved_state()["width"] == 760
...
def test_save_then_load_round_trips():
    geometry = {"width": 500, "height": 700, "x": 40, "y": 60, "on_top": False}
